...
== EVIL_WEEVIL ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libevil_weevil.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── evil_weevil.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── evil_weevil.h       # Generated from Idris2 ABI
│
...
----
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/evil_weevil.h
----
...
[source,c]
----
#include "evil_weevil.h"

int main() {
    void* handle = evil_weevil_init();
    if (!handle) return 1;

    int result = evil_weevil_process(handle, 42);
    if (result != 0) {
        const char* err = evil_weevil_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    evil_weevil_free(handle);
    return 0;
}
----
...
[source,bash]
----
gcc -o example example.c -levil_weevil -L./zig-out/lib
----
...
[source,idris]
----
import EVIL_WEEVIL.ABI.Foreign
...
[source,rust]
----
#[link(name = "evil_weevil")]
extern "C" {
    fn evil_weevil_init() -> *mut std::ffi::c_void;
    fn evil_weevil_free(handle: *mut std::ffi::c_void);
    fn evil_weevil_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = evil_weevil_init();
        assert!(!handle.is_null());

        let result = evil_weevil_process(handle, 42);
        assert_eq!(result, 0);
...
[source,julia]
----
const libevil_weevil = "libevil_weevil"

function init()
    handle = ccall((:evil_weevil_init, libevil_weevil), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:evil_weevil_process, libevil_weevil), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:evil_weevil_free, libevil_weevil), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/evil_weevil.h
   ```
