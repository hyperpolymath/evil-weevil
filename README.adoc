...
[IMPORTANT]
====
*Status: Phase 0 — identity only. There is no kernel code yet.* This repository was
minted from the RSR template on 2026-09-20; what exists today is governance, CI,
machine-readable metadata and an empty, reserved ABI/FFI seam. No behaviour, no
adapter, and no support matrix entry is claimed. `STATE.a2ml` carries the live
number (currently 2%), and `docs/developer/DEVELOPMENT-PLAN.adoc` carries the route.
====
...
| `src/interface/`           | The seam: Idris2 ABI proofs over the Zig FFI, plus generated headers.
| `src/core/`                | The host-agnostic kernel. Empty until Phase 1 exits.
