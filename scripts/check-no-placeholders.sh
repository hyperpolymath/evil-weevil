...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "evil-weevil"`
...
# an instantiation. The template submitted the literal `evil-weevil` as its own
...
# is set to `owner/evil-weevil` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `evil-weevil-2`,
